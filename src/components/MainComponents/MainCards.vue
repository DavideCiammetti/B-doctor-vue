...
<template>
    ...
    <div class="pagination-container mt-5 d-flex justify-content-center align-items-center gap-2">
        ...
        <button @click="goToPage(1)" class="btn text-white"
            :class="{ 'btn-cstm': 1 === currentPage, 'btn-secondary': 1 !== currentPage }">1</button>
        <span v-if="currentPage > 3">...</span>
        <template v-for="page in visiblePages">
            <button v-if="page !== 1 && page !== totalPages" :key="page" @click="goToPage(page)" class="btn text-white"
                :class="{ 'btn-secondary': page !== currentPage, 'btn-cstm': page === currentPage }">{{ page }}</button>
        </template>
        <span v-if="currentPage + 2 < totalPages">...</span>
        <button @click="goToPage(totalPages)" class="btn text-white"
            :class="{ 'btn-cstm': totalPages === currentPage, 'btn-secondary': totalPages !== currentPage }">{{ totalPages
            }}</button>
